...
st.markdown("**3 Models • User Feedback Loop • Content-based Recommender**")
...
def load_and_process_data():
    ...
    # Weighted feature matrix (following your original weights)
    genre_weight = 1.5
    keyword_weight = 2.0
    rating_weight = 0.5
...
hybrid_weight = st.slider("Hybrid popularity boost (recommended: keep small)", 
...
if st.button("🚀 Get Recommendations", type="primary"):
    ...
    rec_df['score'] = [scores[i] for i in filtered]
    rec_df = rec_df.head(15)
    
    st.success(f"✅ Top 15 recommendations using **{model_choice}**")
    
    display_df = rec_df[['title', 'score', 'genres', 'vote_average', 'popularity']].copy()
    ...
    # ====================== FEEDBACK LOOP ======================
    st.subheader("💬 Give Feedback to Improve Recommendations")
    st.caption("👍 Add to liked movies | 👎 Exclude from future recommendations")
    
    for idx, row in rec_df.iterrows():
        col1, col2, col3 = st.columns([7, 1, 1])
        with col1:
            st.write(f"**{row['title']}**")
            st.caption(f"Score: {row['score']:.3f} | Genres: {row['genres']}")
        with col2:
...
st.caption("Project uses your feature engineering + TF-IDF + Embeddings + Feedback Loop")
